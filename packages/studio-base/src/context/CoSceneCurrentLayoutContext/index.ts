...
import { PanelConfig, PlaybackConfig, UserScript } from "@foxglove/studio-base/types/panels";
...
export interface ICurrentLayout {
  ...
  actions: {
    ...
    setUserScripts: (payload: Partial<UserScript>) => void;
...
const CoSceneCurrentLayoutContext = createContext<ICurrentLayout | undefined>(undefined);
CoSceneCurrentLayoutContext.displayName = "CoSceneCurrentLayoutContext";

export function usePanelMosaicId(): string {
  return useGuaranteedContext(CoSceneCurrentLayoutContext).mosaicId;
}
...
export function useSelectedPanels(): SelectedPanelActions {
  ...
  const setSelectedPanelIds = useGuaranteedContext(CoSceneCurrentLayoutContext).setSelectedPanelIds;
  const getSelectedPanelIds = useGuaranteedContext(CoSceneCurrentLayoutContext).getSelectedPanelIds;
...
export default CoSceneCurrentLayoutContext;
