...
import { InitArgs, StateTransitionsChartRenderer } from "./StateTransitionsChartRenderer";
...
// DatalabelPlugin is required for state transition labels
Chart.register(
  LineElement,
  PointElement,
  LineController,
...
Comlink.expose({
  async init(args) {
    await fontLoaded;
    return Comlink.proxy(new StateTransitionsChartRenderer(args));
  },
} satisfies Service<Comlink.LocalObject<StateTransitionsChartRenderer>>);
