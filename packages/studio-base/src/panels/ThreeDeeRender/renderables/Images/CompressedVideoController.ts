...
export class CompressedVideoController {
  ...
  public processMessage(messageEvent: PartialMessageEvent<CompressedVideo>): void {
    ...
    if (frameInfo == undefined) {
      void this.#displayVideoFrame(normalizedEvent);
      return;
    }
  ...
  async #runLookback(
    ...
    for (const lookbackSec of LOOKBACK_WINDOWS_SEC.slice(startIndex)) {
      ...
      const frames = await this.#readLookbackWindow(
        topic,
        state,
        generation,
        lookbackSec,
  ...
  async #displayVideoFrame(
    ...
    try {
      const result = await this.#displayFrame(messageEvent, messageEvent.message);
      if (result == undefined) {
        return { ok: true };
      }
      return result;
...
async function collectReplayableFrames(
  ...
  for await (const batch of iterator) {
    for (const messageEvent of batch) {
      if (messageEvent.topic !== topic || compare(messageEvent.receiveTime, seekTime) > 0) {
        continue;
      }
