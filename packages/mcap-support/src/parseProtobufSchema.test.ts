...
function bytes(value: string, encoding: BufferEncoding): Uint8Array {
...
describe("parseProtobufSchema", () => {
  it("handles protobuf repeated enum having multiple default value aliases", () => {
    ...
    */
    ...
    expect(channel.deserialize(bytes("0A0101", "hex"))).toEqual({ data: [1] });
  });
  ...
  it("converts protobuf time/duration int64 to number rather than bigint", () => {
    ...
    const sceneUpdate = sceneUpdateChannel.deserialize(
      bytes("EhwKDAjHwtyhBhC4nNXrASIMCMfC3KEGELic1esB", "base64"),
    );
    ...
    // Duration too large
    expect(() =>
      sceneUpdateChannel.deserialize(bytes("EhMKBAgCEAMiCwiAgICAgICAEBAB", "base64")),
    ).toThrow(
      "Timestamps with seconds greater than 2^53-1 are not supported (found seconds=9007199254740992, nanos=1)",
    );
    ...
    // Timestamp too large
    expect(() =>
      sceneUpdateChannel.deserialize(bytes("EhMKCwiAgICAgICAEBABIgQIAhAD", "base64")),
    ).toThrow(
      "Timestamps with seconds greater than 2^53-1 are not supported (found seconds=9007199254740992, nanos=1)",
    );
  });
